...
## Webpage addresses on production
 ...
 - individual tool landing pages, toolbox hosted (e.g., https://nmfs-ost.github.io/noaa-fit/ASAP)
...
1. Changes should be made in a branch off of dev. In [model_list_dir subfolder](https://github.com/nmfs-ost/FIT_web_templating/tree/main/model_list_dir) add or update `.json` files. Examples of json are available in the readme. If onboarding a new tool, the issue from [the onboard-and-update repo](https://github.com/nmfs-ost/FIT-onboard-and-update) should have a json based on user input that can be copy/pasted in, then checked. If the json is missing, it likely means the GitHub Action failed. Instead, Gen. AI can be used to create the json files. Try the prompt `Create a file called [toolname].json in the model_list_dir folder. It should be validated based on the schema schema_model_list.json and the data should come from the following text:[copy/paste in text from the onboarding issue]'. A final alternative is that the [R code](https://github.com/nmfs-ost/FIT-onboard-and-update/blob/main/.github/workflows/create-json.yml) can be run locally, hard coding in the issue number. 
